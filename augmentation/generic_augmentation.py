...
def random_crop_image_4D(img, tree, soma, target_shape):
    ...
    # processing the gt
    for c in range(soma.shape[0]):
        new_soma[c] = soma[c][sz:sz+target_shape[0], sy:sy+target_shape[1], sx:sx+target_shape[2]]
...
class RandomSaturation(AbstractTransform):
    ...
    def __call__(self, img, gt=None, spacing=None):
...
class CLAHETransform(AbstractTransform):
    ...
    def __call__(self, img, tree=None, soma_pad=None, spacing=None):
        ...
        return img[None], tree, soma_pad, spacing
    ...
    def __init__(self, p=0.2, imgshape=(48,96,96), phase='train', divid=2**5):
        if phase == 'train':
            ...
            self.augment = Compose([
                ConvertToFloat(),
                RandomCrop(1.0, imgshape, crop_range=(0.85, 1.2)),
