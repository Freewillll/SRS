...
class MostFitCropEvaluation(GenericMultiCropEvaluation):
    ...
    def get_image_crops(self, img, lab=None):
        ...
        crops = []
        size_z_lab = size_z * 4
        crop_lab_size = (size_z_lab, size_y, size_x)
        for zi in range(int(math.ceil(nshape[0] / size_z))):
            zs, ze = zi * size_z, (zi+1) * size_z
            zs_lab, ze_lab = zi*size_z_lab, (zi+1) * size_z_lab
            if ze > nshape[0]:
                ze = None
                zs = -size_z
            for yi in range(int(math.ceil(nshape[1] / size_y))):
                ...
                for xi in range(int(math.ceil(nshape[2] / size_x))):
                    ...
                    crops.append(crop)
                    if lab is not None:
                        lab_crop = pad_lab[zs_lab:ze_lab, ys:ye, xs:xe]
                        lab_crops.append(lab_crop)
        
        return crops, crop_sizes, lab_crops, crop_lab_size
...
class OverlapCropEvaluation(GenericMultiCropEvaluation):
    def __init__(self, patch_size, divid=(2**4,2**5,2**5), pad_value='min'):
        super(OverlapCropEvaluation, self).__init__(patch_size, divid, pad_value)
        self.overlap = 0
    ...
    def get_crop_sizes(self, imgshape):
        ...
        return crop_sizes, pads, overlap

    def get_image_crops(self, img, lab=None):
        # image in shape [c, z, y, x]
        imgshape = img[0].shape
        crop_sizes, img_pads, overlap = self.get_crop_sizes(imgshape)
        ...
        crops = []
        size_z_lab = size_z * 4
        lab_overlap_z = 4 * self.overlap[0]
        crop_lab_size = (size_z_lab, size_y, size_x)
        for zi in range(int(math.ceil(nshape[0] / size_z) + 1)):
            zs, ze = zi*size_z - zi*self.overlap[0], (zi+1)*size_z - zi*self.overlap[0]
            zs_lab, ze_lab = zi*size_z_lab -zi*lab_overlap_z, (zi+1)*size_z_lab -zi*lab_overlap_z
            if ze > nshape[0]:
                ze = None
                zs = -size_z
            for yi in range(int(math.ceil(nshape[1] / size_y) + 1)):
                ys, ye = yi * size_y - yi*self.overlap[1], (yi+1) * size_y - yi*self.overlap[1]
                if ye > nshape[1]:
                    ye = None
                    ys = -size_y

                for xi in range(int(math.ceil(nshape[2] / size_x) + 1)):
                    xs, xe = xi * size_x - xi*self.overlap[2], (xi+1) * size_x- xi*self.overlap[2]
                    if xe > nshape[2]:
                        xe = None
                        xs = -size_x
                    # print(zi,yi,xi,zs,ze,ys,ye,xs,xe,pad_img.shape)
                    ...
                    crops.append(crop)
                    if lab is not None:
                        lab_crop = lab[zs_lab:ze_lab, ys:ye, xs:xe]
                        lab_crops.append(lab_crop)
    ...
    def get_pred_from_crops(self, imgshape, preds, crop_sizes):
        ...
        ncrops = [int(math.ceil(imgshape[i]/preds[0][0].shape[i])) for i in range(3)]
        if len(ncrops) >1:
            lab_overlap_z = self.overlap[0] * 4
            ncrops[0] += 1
            ncrops[1] += 1
            ncrops[2] += 1
        ...
        for zi in range(ncrops[0]):
            zs, ze = zi * size_z - zi*lab_overlap_z , (zi+1) * size_z - zi*lab_overlap_z
            if zi >= ncrops[0]:
                ze = None
                zs = -size_z
            ...
            for yi in range(ncrops[1]):
                ...
                for xi in range(ncrops[2]):
                    ...
                    #NOTE: it is definitely better to combines results for overlap regions. to be optimized later.    
                    idx = xi + yi * ncrops[2] + zi * ncrops[1] * ncrops[2]
                    # print(zs, ze, ys, ye, xs, xe)
                    pred_pre = torch.clone(pred[:, zs:ze, ys:ye, xs:xe])
                    pred_tmp = preds[idx]
                    pred_all = torch.unsqueeze(pred_pre, 0).repeat(2, 1, 1, 1, 1)
                    pred_all[1] = pred_tmp
                    pred_final = torch.max(pred_all, dim=0)[0]
                    pred[:, zs:ze, ys:ye, xs:xe] = pred_final
...
if __name__ == '__main__':
    ...
    lab = torch.rand((512, 256, 256)) > 0.5

    imgshape = lab.shape
    oce = OverlapCropEvaluation(patch_size)
    crops, crop_sizes, lab_crops, crop_lab_size = oce.get_image_crops(img, lab)
    ...
    for i, crop, lab_crop in zip(range(len(crops)), crops, lab_crops):
        ...
        preds.append(lab_crop[None])

    pred = oce.get_pred_from_crops(imgshape, preds, crop_lab_size)
    print(pred.shape)